...
@section('content')
    <div class="flex-1 p-6 overflow-auto max-h-[calc(100vh-150px)]">
        ...
        <div class="bg-white shadow-md rounded-lg p-6 ">
            ...
            <form action="{{ route('admin.umkm.update', ['id' => $umkm->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="nama" name="nama" value="{{ $umkm->nama }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                    @error('nama')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="pemilik" name="pemilik" value="{{ $umkm->pemilik }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                    @error('pemilik')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="no_hp" name="no_hp" value="{{ $umkm->no_hp }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                    @error('no_hp')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="6"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>{{ $umkm->deskripsi }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    @enderror
                    @if ($umkm->gambar)
                        <img src="{{ asset('storage/umkm_images/' . $umkm->gambar) }}" alt="Gambar UMKM"
